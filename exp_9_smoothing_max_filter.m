...
padded_img(1:p, p+1:end-p) = only_pepper_noise(1:p,:);
padded_img(p+1:end-p, 1:p) = only_pepper_noise(:,1:p); 
padded_img(end-p:end, p+1:end-p) = only_pepper_noise(end-p:end, :);
padded_img(p+1:end-p,end-p:end) = only_pepper_noise(:,end-p:end);

padded_img(1:p,1:p) = only_pepper_noise(1:p,1:p);
padded_img(1:p, end-p:end) = only_pepper_noise(1:p, end-p:end);
padded_img(end-p:end, 1:p) = only_pepper_noise(end-p:end,1:p);
padded_img(end-p:end, end-p:end) = only_pepper_noise(end-p:end,end-p:end);
